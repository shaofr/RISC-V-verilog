...
module HarzardUnit(
    ...
    always@(*)begin
        flushF<=rst;
        flushD<=rst||(jal||jalr||br);
        //只有在load指令时，因为是访问mem之后才知道结果，�?以不能像alu�?样转发，必须在写回地�?与后续指令源操作数地�?相同时停�?
        flushE<=rst||(wb_select&&(reg_dstE==reg1_srcD||reg_dstE==reg2_srcD))||br||jalr;
        flushM<=rst;
        flushW<=rst;
    ...
    always@(*)begin
        if((reg_write_en_MEM==1'b1)&&(reg_dstM==reg1_srcE)&&(reg_dstM!=5'b0)&&(src_reg_en[1]))//todo
            op1_sel<=2'b00;
        else if((reg_write_en_WB==1'b1)&&(reg_dstW==reg1_srcE)&&(reg_dstW!=5'b0)&&(src_reg_en[1]))
            op1_sel<=2'b01;
    ...
    always@(*)begin
        if((reg_write_en_MEM==1'b1)&&(reg_dstM==reg2_srcE)&&(reg_dstM!=5'b0)&&(src_reg_en[0])&&(alu_src2!=2'b10))//todo
            op2_sel<=2'b00;
        else if((reg_write_en_WB==1'b1)&&(reg_dstW==reg2_srcE)&&(reg_dstW!=5'b0)&&(src_reg_en[0])&&(alu_src2!=2'b10))
            op2_sel<=2'b01;
        else if(alu_src2==2'b01)
            op2_sel<=2'b10;
        else if(alu_src2==2'b10||alu_src2==2'b00)
            op2_sel<=2'b11;
    end

    always@(*)begin
        if((reg_write_en_MEM==1'b1)&&(reg_dstM==reg2_srcE)&&(reg_dstM!=5'b0)&&(src_reg_en[0]))
            reg2_sel<=2'b00;
        else if((reg_write_en_WB==1'b1)&&(reg_dstW==reg2_srcE)&&(reg_dstW!=5'b0)&&(src_reg_en[0]))
            reg2_sel<=2'b01;
        else
            reg2_sel<=2'b10;
    end
